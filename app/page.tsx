  ...
  return (
    <div>
      ...
      <section className="section" style={{ paddingTop: "3.5rem", paddingBottom: "3rem" }}>
        <div className="hero-grid" style={{ display: "grid", gridTemplateColumns: "1.05fr 0.95fr", gap: "3.5rem", alignItems: "center" }}>
          <div>
            ...
            <Reveal delay={60}>
              <h1 className="display" style={{ fontSize: "clamp(2.9rem, 6.6vw, 5.4rem)", marginTop: "1.35rem" }}>
                Personal styling,
                <br />
                made <em>personal.</em>
              </h1>
            ...
            <Reveal delay={120}>
              <p className="lede" style={{ maxWidth: 470, marginTop: "1.6rem" }}>
            ...
            <Reveal delay={240}>
              <div style={{ display: "flex", gap: "2.75rem", marginTop: "2.5rem", flexWrap: "wrap" }}>
                {[
                  ...
                  <div key={m.label}>
                    <div className="font-serif" style={{ fontSize: "1.8rem", fontWeight: 500 }}>{m.stat}</div>
                    <div style={{ color: "var(--faint)", fontSize: "0.82rem", letterSpacing: "0.02em" }}>{m.label}</div>
                  </div>
                ))}
              </div>
            </Reveal>
          ...
          <Reveal delay={120} className="hero-visual" style={{ position: "relative" }}>
            <div className="photo" style={{ borderRadius: 18, aspectRatio: "4 / 5", boxShadow: "0 30px 70px rgba(29,26,21,0.18)" }}>
              <Image
            ...
            <div className="card" style={{ position: "absolute", bottom: 20, left: -20, padding: "0.9rem 1.15rem", display: "flex", alignItems: "center", gap: "0.7rem" }}>
              <span className="font-serif" style={{ fontStyle: "italic", fontSize: "1.5rem", color: "var(--accent-dark)" }}>✦</span>
              <div>
                <div style={{ fontWeight: 600, fontSize: "0.9rem" }}>Hand-vetted stylists</div>
                <div style={{ color: "var(--faint)", fontSize: "0.76rem" }}>By appointment, worldwide</div>
              </div>
            </div>
          </Reveal>
        </div>
      ...
      <section className="section">
        <div style={{ display: "flex", flexWrap: "wrap", gap: "0 0.4rem", justifyContent: "center", padding: "1.4rem 0", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)" }}>
          {SPECIALTIES.map((s, i) => (
            <span key={s} style={{ display: "inline-flex", alignItems: "center", color: "var(--dim)", fontSize: "0.86rem", padding: "0.25rem 0.7rem" }}>
              {s}
              {i < SPECIALTIES.length - 1 && <span style={{ color: "var(--faint)", marginLeft: "0.7rem" }}>·</span>}
            </span>
          ))}
        </div>
      </section>
          ...
          {STEPS.map((step, i) => (
            <Reveal key={step.n} delay={i * 90}>
              <div className="card" style={{ padding: "1.9rem", height: "100%" }}>
                <div className="font-serif" style={{ fontSize: "1.9rem", color: "var(--accent-dark)", fontWeight: 500, lineHeight: 1 }}>{step.n}</div>
                <h3 className="font-serif" style={{ fontSize: "1.3rem", fontWeight: 500, marginTop: "0.9rem" }}>{step.title}</h3>
                <p style={{ color: "var(--dim)", marginTop: "0.55rem" }}>{step.body}</p>
              </div>
            </Reveal>
          ))}
        </div>
      ...
      <section className="section" style={{ padding: "1rem 1.5rem 4rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(270px, 1fr))", gap: "1.5rem", marginTop: "2.25rem" }}>
      ...
      <section className="section" style={{ padding: "1rem 1.5rem 5rem" }}>
        <Reveal>
          <div style={{ position: "relative", borderRadius: 20, overflow: "hidden", border: "1px solid var(--ink)" }}>
            <div className="photo" style={{ position: "absolute", inset: 0 }}>
              <Image src="https://images.unsplash.com/photo-1490481651871-ab68de25d43d?w=1400&q=80" alt="" fill sizes="100vw" style={{ objectFit: "cover" }} />
            </div>
            <div style={{ position: "absolute", inset: 0, background: "rgba(29,26,21,0.62)" }} />
            <div style={{ position: "relative", padding: "4rem 2rem", textAlign: "center", color: "#fff" }}>
              <span className="eyebrow" style={{ color: "rgba(255,255,255,0.7)", justifyContent: "center" }}>Free · two minutes</span>
              <h2 className="display" style={{ fontSize: "clamp(2.1rem, 5vw, 3.4rem)", marginTop: "0.9rem" }}>
                Not sure where to start?
              </h2>
              <p style={{ maxWidth: 520, margin: "0.9rem auto 0", color: "rgba(255,255,255,0.85)", fontSize: "1.05rem" }}>
                Discover your colour season and get a personalised palette plus matched stylists.
              </p>
              <Link href="/quiz" className="btn" style={{ marginTop: "1.8rem", background: "var(--paper)", color: "var(--ink)" }}>
